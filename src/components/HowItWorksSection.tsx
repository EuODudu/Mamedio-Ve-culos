  ...
  return (
    <section className="section-padding bg-background relative">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-7 lg:gap-8 relative">
          {/* Connection Line - Apenas desktop */}
          ...
          {steps.map((step, index) => (
            ...
            >
              {/* Step Number - Mobile: menor */}
              <div className="relative mb-4 md:mb-5 lg:mb-6">
                <div className="w-16 h-16 sm:w-18 sm:h-18 md:w-20 md:h-20 rounded-xl md:rounded-2xl bg-card border border-border flex items-center justify-center group-hover:border-accent/50 transition-all duration-300 group-hover:shadow-lg">
                  <step.icon className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-accent" />
                </div>
                <div className="absolute -top-2 -right-2 md:-top-3 md:-right-3 w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-accent text-accent-foreground flex items-center justify-center text-xs sm:text-xs md:text-sm font-bold">
                  {index + 1}
                </div>
              </div>

              {/* Content */}
              <h3 className="text-lg sm:text-xl font-bold mb-2 md:mb-2.5 lg:mb-3 group-hover:text-accent transition-colors">
                {step.title}
              </h3>
